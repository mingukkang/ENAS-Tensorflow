...
def _read_data(data_path, channel, img_size, n_aug_img):
    ...
    if aug_flag is True:
        ...
        idx = np.random.RandomState(seed = 42).permutation(length_data)
    ...
    mean = np.mean(images, axis=(0,1,2)).astype(np.float32)
    std = np.std(images, axis=(0, 1, 2)).astype(np.float32)
...
def img_augmentation(image):
    ...
    def random_bright_contrast(image):
        alpha = random.uniform(0.6, 1.0) # for contrast
        beta = random.uniform(32, 64) # for brightness
    ...
    def gaussian_noise(image):
        ...
        noise = np.random.normal(size=np.shape(image), loc =0.0, scale= 16)
        noise = np.maximum(noise,np.zeros_like(image))
    ...
    def aug(image, idx):
        augmentation_dic = {0: flip(image),
                            1: enlarge(image, 1.2),
                            2: rotation(image),
                            3: random_bright_contrast(image),
                            4: gaussian_noise(image)}
